...
const Inbox: React.FC = () => {
  ...
  const { loading, error, data, refetch } = useQuery<GetValidationInboxResponse>(
    GET_VALIDATION_INBOX_QUERY
  );
  ...
  useEffect(() => {
    if (data?.getValidationInbox.projects && data.getValidationInbox.projects.length > 0) {
  ...
  const goToNextSuggestion = () => {
    ...
    if (selectedEmployee && currentSuggestionIndex < selectedEmployee.employee.suggestions.length - 1) {
  ...
  const currentSuggestion = selectedEmployee?.employee.suggestions[currentSuggestionIndex];
      ...
      <div className="relative z-10 px-4 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-400 mt-2">Review pending skill suggestions for your team</p>
          </div>
                          ...
                          <div className="ml-4 mt-2 space-y-2">
                            {project.employees.map((employee) => (
                              <button
                                ...
                                onClick={() => selectEmployee(project.projectId, employee.employeeId)}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-4 pb-6 border-b border-gray-700/50">
                      <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white font-bold text-2xl">
                          {selectedEmployee.employee.employeeName.charAt(0).toUpperCase()}
                        </span>
                      </div>
                      <div>
                        ...
                        <p className="text-gray-400">{selectedEmployee.employee.employeeEmail}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <p className="text-sm text-gray-400">
                          Suggestion {currentSuggestionIndex + 1} of {selectedEmployee.employee.suggestions.length}
                        </p>
                      </div>
                      ...
                      <div className="bg-gray-700/30 rounded-lg p-6 border border-gray-600/50">
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getDisciplineColor(currentSuggestion.discipline)}`}>
                          ...
                          {/* Current Proficiency */}
                          <div>
                            <p className="text-sm text-gray-400 mb-1">Current Proficiency</p>
                            {currentSuggestion.currentProficiency ? (
                              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-600/30 text-gray-300">
                                {currentSuggestion.currentProficiency}
                              </span>
                            ) : (
                              <p className="text-gray-500 text-sm">No current proficiency</p>
                            )}
                          </div>

                          {/* Suggested Proficiency */}
                          <div>
                            <p className="text-sm text-gray-400 mb-1">Suggested Proficiency</p>
                            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-600/30 text-gray-300">
                          ...
                          <div>
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getSourceColor(currentSuggestion.source)}`}>
                          ...
                          <div>
                            ...
                            <p className="text-gray-300 text-sm">{formatDate(currentSuggestion.createdAt)}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-3 mt-6 pt-6 border-t border-gray-700/50">
                          ...
                          <button
                            ...
                            disabled={currentSuggestionIndex === selectedEmployee.employee.suggestions.length - 1}
                            className="flex-1 px-4 py-2 bg-gray-700/50 hover:bg-gray-600/50 disabled:bg-gray-700/20 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors border border-gray-600/50"
                          >
                            Next Suggestion
                          </button>
                    ...
                    <div className="flex gap-3 pt-6 border-t border-gray-700/50">
                      ...
                      <button
                        ...
                        disabled={currentEmployeeIndex === flattenedEmployees.length - 1}
                        className="flex-1 px-4 py-2 bg-gray-700/50 hover:bg-gray-600/50 disabled:bg-gray-700/20 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors border border-gray-600/50"
                      >
                        Next Person
                      </button>
